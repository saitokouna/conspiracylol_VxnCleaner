...
#define SPOOF_FUNC_WITH_ADDR(addr) CallSpoofer::SpoofFunction spoof(_AddressOfReturnAddress(), addr);
#ifdef _KERNEL_MODE
...
namespace CallSpoofer
{
    ...
    public:
        ...
        SpoofFunction(void* addr, uintptr_t custom_fake_addr = 0) : ret_addr_in_stack(addr)
        {
            if (custom_fake_addr)
            {
                temp = custom_fake_addr;
            }
            else
            {
